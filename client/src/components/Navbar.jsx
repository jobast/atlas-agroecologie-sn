...
export default function Navbar() {
  ...
  // Pending offline submissions count, kept fresh via the queue event.
  useEffect(() => {
    let cancelled = false;
    const refresh = async () => {
      try {
        const n = await pendingCount();
        if (!cancelled) setPendingN(n);
    ...
    <nav className="bg-white/90 backdrop-blur-md shadow-md fixed top-0 left-0 right-0 z-50">
      <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          {token && pendingN > 0 && (
            <button
              onClick={() => go(p('/mes-envois-en-attente'))}
              className="inline-flex items-center gap-1.5 text-xs font-medium bg-amber-100 hover:bg-amber-200 text-amber-800 px-2.5 py-1 rounded-full transition-colors"
              title={t('offline.pendingBadgeTitle', { defaultValue: 'Envois en attente' })}
            >
              <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="12" cy="12" r="10"/>
                <polyline points="12 6 12 12 16 14"/>
              </svg>
              {t('offline.pendingBadge', { count: pendingN, defaultValue: '{{count}} en attente' })}
            </button>
          )}
          <InstallButton className="inline-flex items-center gap-1.5 text-xs font-medium bg-emerald-50 hover:bg-emerald-100 text-emerald-800 px-2.5 py-1 rounded-md border border-emerald-200/60 transition-colors" />
          <button
            onClick={toggleLang}
      ...
      {menuOpen && (
        <div className="md:hidden fixed inset-0 z-40" style={{ top: '3.5rem' }}>
          ...
          <div className="relative bg-white border-b border-stone-200 shadow-lg p-4 space-y-1">
            ...
            {/* Field-agent helpers: pending queue + install + onboarding */}
            {token && (
              <div className="pt-2 mt-2 border-t border-stone-100 space-y-1">
                <span className="block px-3 text-[10px] uppercase tracking-wider text-stone-400">
                  {t('nav.field_section', { defaultValue: 'Mode terrain' })}
                </span>
                {pendingN > 0 && (
                  <button
                    onClick={() => go(p('/mes-envois-en-attente'))}
                    className="block w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors text-amber-800 bg-amber-50 hover:bg-amber-100"
                  >
                    {t('offline.pendingBadge', { count: pendingN, defaultValue: '{{count}} en attente' })}
                  </button>
                )}
                <button
                  ...
                  className="block w-full text-left px-3 py-2.5 rounded-lg text-sm text-stone-600 hover:bg-stone-50"
                >
                  {t('onboarding.menuEntry', { defaultValue: 'Guide enquêteur' })}
                </button>
                <div className="px-2 pt-1">
                  <InstallButton className="w-full inline-flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2 rounded-lg text-sm font-medium" />
                </div>
              </div>
            )}
