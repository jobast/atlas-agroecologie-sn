  ...
  const actions = [
    {
      ...
      description: t('landing.explore_map_desc'),
    ...
    {
      label: t('landing.view_table'),
      description: t('landing.view_table_desc'),
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-12 relative z-10">
        ...
        <div className="text-center mb-12 max-w-lg">
          <div className="inline-flex items-center gap-2 mb-4 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200/60">
            ...
            <span className="text-xs font-medium text-emerald-700 tracking-wide">
              {isNational ? t('landing.dytaes_national') : t('landing.dytael_name', { name: titleSuffix })}
            </span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-light text-stone-800 tracking-tight mb-3">
            {isNational ? t('landing.atlas_agroecologique') : titleSuffix}
          </h1>
        ...
        {isAdmin && (
          <div className="mt-6 pt-6 border-t border-stone-200/60 w-full max-w-lg">
            <p className="text-[10px] text-stone-400 uppercase tracking-widest mb-3 text-center">{t('nav.administration')}</p>
            <div className="flex justify-center gap-2">
              {[
                { label: t('nav.data'), path: p('/admin'), icon: 'M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-9.75 0h9.75' },
                { label: t('nav.users'), path: p('/users'), icon: 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z' },
                { label: t('nav.form'), path: p('/form-fields'), icon: 'M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25z' },
              ].map((item) => (
                <button
                  key={item.label}
